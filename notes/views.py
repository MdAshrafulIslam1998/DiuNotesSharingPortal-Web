...
def log(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = "yes"
    r = {'error': error}
    return render(request, 'login.html', r)
...
def signup1(request):
    ...
    if request.method=='POST':
      ...
      k = request.POST['campus']
      r = request.POST['role']
      try:
        user = User.objects.create_user(username=e,password=p,first_name=f,last_name=l,is_staff=True)
        Signup.objects.create(user=user, contact=c, branch=b,campus=k, role=r)
        error="no"
      except:
        error="yes"
    d= {'error':error}
...
def upload_notes(request):
    ...
    if request.method=='POST':
      b = request.POST['branch']
      r = request.POST['role1']
      c = request.POST['campus1']
      ...
      try:
        Notes.objects.create(user=u,uploadingdate=date.today(),branch=b,role1=r,campus1=c,subject=s,topic=t,notesfile=n,filetype=f,description=d,status='pending')
